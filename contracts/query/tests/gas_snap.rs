...
/// `get_protocol_summary` after opening 5 lines reflects the accumulator.
#[test]
...
/// `is_delinquent` with no repayment schedule returns `false` cheaply.
#[test]
...
/// `is_delinquent` with a schedule not yet due returns `false`.
#[test]
...
/// `get_health_factor` with zero utilization returns `u32::MAX`.
#[test]
...
/// `get_health_factor` with utilization and no collateral returns 0.
#[test]
...
/// `get_repayment_schedule` with no schedule returns `None`.
#[test]
...
/// `get_repayment_schedule` after setting one returns the stored value.
#[test]
fn gas_get_repayment_schedule_configured() {
    ...
    let (cpu, mem) = measure(&env, || {
        ...
        assert!(result.is_some());
        assert_eq!(result.unwrap().amount_per_period, 2_000_i128);
    });
...
fn gas_query_deterministic_borrow_capabilities() {
    ...
    let (cpu1, mem1) = measure(&env, || { client.borrow_capabilities(&borrowers[0]); });
    let (cpu2, mem2) = measure(&env, || { client.borrow_capabilities(&borrowers[0]); });
