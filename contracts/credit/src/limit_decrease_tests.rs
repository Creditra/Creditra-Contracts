...
fn test_draw_blocked_when_restricted() {
    ...
    assert!(
        result.is_err(),
        "Expected draw to be blocked in Restricted status"
    );
}
...
fn test_auto_cure_when_limit_increased_to_meet_utilization() {
    ...
    assert_eq!(
        line.status,
        CreditStatus::Active,
        "Status should auto-cure to Active"
    );
}
...
fn test_multiple_restriction_and_cure_cycles() {
    ...
    assert_eq!(
        client.get_credit_line(&borrower).status,
...
fn test_restriction_partial_repay_still_restricted() {
    ...
    assert_eq!(
        line.status,
        CreditStatus::Restricted,
        "Still restricted since 6000 > 5000"
    );
