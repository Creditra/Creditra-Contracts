...
fn settle_within_deviation_bound_accepted() {
    ...
    client.settle_default_liquidation(&b1, &200_i128, &sid(&env, "s1"), &10_000_u32, &Some(1_000_i128));

    // Second settlement — price 1_040 is 4% deviation from 1_000 (within 5%).
    let b2 = open_and_default(&client, &env, &contract_id, 200);
    client.settle_default_liquidation(&b2, &200_i128, &sid(&env, "s2"), &10_000_u32, &Some(1_040_i128));
...
fn settle_over_deviation_panics() {
    ...
    let b2 = open_and_default(&client, &env, &contract_id, 200);
    client.settle_default_liquidation(&b2, &200_i128, &sid(&env, "s2"), &10_000_u32, &Some(1_100_i128));
}
...
fn settle_over_deviation_downward_panics() {
    ...
    let b2 = open_and_default(&client, &env, &contract_id, 200);
    client.settle_default_liquidation(&b2, &200_i128, &sid(&env, "s2"), &10_000_u32, &Some(900_i128));
}
...
fn settle_stale_price_panics() {
    ...
    let b2 = open_and_default(&client, &env, &contract_id, 200);
    client.settle_default_liquidation(&b2, &200_i128, &sid(&env, "s2"), &10_000_u32, &Some(1_010_i128));
}
...
fn settle_zero_oracle_price_panics() {
    ...
    client.settle_default_liquidation(&borrower, &200_i128, &sid(&env, "s1"), &10_000_u32, &Some(0_i128));
}
...
fn settle_negative_oracle_price_panics() {
    ...
    client.set_oracle_config(&500_u32, &3600_u64);
    let borrower = open_and_default(&client, &env, &contract_id, 200);
    client.settle_default_liquidation(&borrower, &200_i128, &sid(&env, "s1"), &10_000_u32, &Some(-1_i128));
}
