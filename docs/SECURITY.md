...
| T9 | **Admin compromise → drain treasury / drain reserve.** | Treasury withdrawals require `propose_treasury_withdrawal` followed by `confirm_treasury_withdrawal` after a fixed 24-hour delay. Confirmation can move only the proposed amount from `TreasuryBalance`; it cannot touch the external liquidity reserve or borrower funds. | `lib.rs` treasury entrypoints |
